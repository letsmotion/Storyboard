...
public class AIConfigurationComposer
{
    ...
    public void SaveUserConfiguration(string providerName, ProviderUserConfig userConfig)
    {
        ...
        // 如果�?有字段都为空（除�?ApiKey），则完全移除该 Provider 配置
            ...
            diffConfig.DefaultModels == null)
        {
            ...
            _logger.LogInformation("用户配置已移�?(无差�?: {Provider}", providerName);
        }
        else
        {
            overrides.Providers[providerName] = diffConfig;
            _logger.LogInformation("用户配置已保�?(仅差�?: {Provider}", providerName);
        }
    ...
    private void LoadSchema()
    {
        ...
        {
            _logger.LogWarning("Provider Capability Schema 文件不存�? {Path}", schemaPath);
    ...
    private void LoadDefaults()
    {
        ...
        {
            _logger.LogWarning("AI Defaults 文件不存�? {Path}", defaultsPath);
    ...
    private void ValidateUserOverrides(UserAIOverrides userOverrides)
    {
        ...
        {
            _logger.LogWarning("移除未知�?Provider 配置: {Provider}", unknownProvider);
